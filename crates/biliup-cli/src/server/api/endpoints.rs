...
use crate::server::infrastructure::context::{Stage, Worker, WorkerStatus, WorkerStatusSnapshot};
...
fn download_status_snapshot(worker: &Worker) -> WorkerStatusSnapshot {
    worker
        .downloader_status
        .read()
        .map(|guard| guard.snapshot())
        .unwrap_or(WorkerStatusSnapshot {
            raw_status: "Poisoned",
            display_status: "Poisoned",
            is_stale: false,
...
pub async fn get_streamers_endpoint(
    ...
    for x in live_streamers {
        ...
        let status = option
            .as_ref()
            .map(|worker| download_status_snapshot(worker).display_status.to_string())
            .unwrap_or_default();
...
pub async fn get_status(
    ...
    for worker in &workers {
        let download_status = download_status_snapshot(worker);
        sw.push(serde_json::json!({
            "downloader_status": download_status.display_status,
            "downloader_status_raw": download_status.raw_status,
            "downloader_status_stale": download_status.is_stale,
            "downloader_last_activity_at": download_status.last_activity_at,
            "downloader_stale_after_secs": download_status.stale_after_secs,
